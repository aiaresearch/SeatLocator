...
@app.route('/query')
def seat_query():
    latest_seat_data_1 = SeatData_1.query.order_by(SeatData_1.id.desc()).first()
    latest_seat_data_2 = SeatData_2.query.order_by(SeatData_2.id.desc()).first()
    ...
    if request.headers.get('Accept') == 'application/json':
        if latest_seat_data_1 and latest_seat_data_2:
            ...
            return jsonify({'error': '暂无座位数据'}), 404
    else:
...
def update_data_1():
    ...
    if provided_password == password:
        ...
        return jsonify({'error': '密码错误，连接拒绝'}), 401
...
def update_data_2():
    ...
    if provided_password==password:
        ...
        return jsonify({'error': '密码错误，请求拒绝'}), 401
